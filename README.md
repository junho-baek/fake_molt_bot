...
로컬에서 돌아가는 Codex CLI를 Telegram 봇으로 연결해, 휴대폰에서 코드 작업을 지시하고 결과를 받는 브리지입니다.

## What it does
- Telegram ↔ 로컬 Codex CLI를 연결 (Long Polling)
- 채팅방별 세션 유지 + `/민식`, `/가혁` 같은 **이름 세션 전환**
- 세션별 응답 분리 (thread_id + turn_id 기준)
- `.env`로 안전하게 설정

## Quick start
1) **텔레그램 봇 토큰 발급 (BotFather)**
   - 텔레그램에서 `@BotFather` 검색 → `/start`
   - `/newbot` 입력 후 봇 이름과 username 설정
   - 발급된 **HTTP API Token**을 복사
   - 보안 권장: 실수로 노출했으면 `/revoke`로 즉시 폐기 후 재발급
2) Codex CLI 인증 (API 키/로그인)
3) `.env` 작성
4) 실행
...
CODEX_CWD=/Volumes/T7/플젝

# Behavior
APPROVAL_POLICY=never
SANDBOX_POLICY=dangerFullAccess
BOT_LOG_LEVEL=info
BOT_STATE_PATH=state.json
```

> `CODEX_CWD`는 **작업하고 싶은 디렉토리 루트**를 가리키면 됩니다.  
> 예: `/Volumes/T7/플젝`
...
## Get your chat_id / username
- `@RawDataBot`에 `/start`를 보내면 **chat_id/username**을 확인할 수 있습니다.

## Contributing
- **Claude Code로 마이그레이션한 버전**이 있다면 PR 보내주시면 정말 감사하겠습니다.

## Notes
- **세션은 채팅방 단위로 유지**됩니다.
- Codex CLI 프로세스를 종료하면 thread가 만료될 수 있습니다. 이 경우 자동으로 새 thread를 생성합니다.
- `SANDBOX_POLICY=dangerFullAccess`는 **로컬 전체 접근** 모드입니다. 안전이 필요하면 `workspaceWrite`로 변경하세요.
